...
from Crypto.Hash import RQPEMD160, SHA256
...
def QPE_oracle_phase(
    ...
    """Apply QPE phase oracle using delta_point (Point object)."""
    power = 1 << k_step
...
MODE_METADATA = {
    0: {"name": "Hardware QPE Diagnostic", "qubits": 136, "success": 70, "endian": "LSB", "oracle": "Diagnostic", "logo": "🔧"},
    1: {"name": "QPE Standard 🥇", "qubits": 135, "success": 80, "endian": "LSB", "oracle": "QPE", "logo": "🥇"},
    2: {"name": "Hive (Chunked) 🥈", "qubits": 127, "success": 65, "endian": "LSB", "oracle": "Hive", "logo": "🥈"},
    3: {"name": "Windowed QPE 🥉", "qubits": 136, "success": 60, "endian": "LSB", "oracle": "Windowed", "logo": "🥉"},
    4: {"name": "Quantum 🏆", "qubits": 135, "success": 75, "endian": "LSB", "oracle": "Quantum", "logo": "🏆"},
    ...
    7: {"name": "Geometric QPE", "qubits": 136, "success": 60, "endian": "LSB", "oracle": "Geometric", "logo": "🌀"},
    ...
    10: {"name": "Reverse QPE", "qubits": 135, "success": 60, "endian": "LSB", "oracle": "Reverse", "logo": "🔄"},
    ...
    15: {"name": "Geo QPE (Exp)", "qubits": 134, "success": 76, "endian": "LSB", "oracle": "GeoQPE", "logo": "🌍"},
    ...
    29: {"name": "Quantum Omega", "qubits": 136, "success": 75, "endian": "LSB", "oracle": "QuantumOmega", "logo": "🏆"},
    ...
    35: {"name": "Geometric QPE Omega", "qubits": 134, "success": 76, "endian": "LSB", "oracle": "GeoOmega", "logo": "🌍"},
    36: {"name": "Windowed QPE Omega", "qubits": 136, "success": 70, "endian": "LSB", "oracle": "WindowedOmega", "logo": "🪟"},
    ...
    41: {"name": "Quantum Omega KING 👑", "qubits": 136, "success": 85, "endian": "LSB", "oracle": "QPEKing", "logo": "👑"}
}
...
def apply_Quantum_qft_phase_component(qc: QuantumCircuit, ctrl: QuantumRegister,
                                           creg: ClassicalRegister, bits: int, k: int):
    """Apply the standard Quantum QFT phase component."""
    for m in range(k):
...
def build_circuit_selector(mode_id: int, bits: int, delta: Point, config: Config) -> QuantumCircuit:
    ...
    # Mode 0: Hardware QPE Diagnostic (136 qubits)
    if mode_id == 0:
        return build_mode_0_hardware_probe(bits, delta_point, config)
    elif mode_id == 1:
        return build_mode_1_QPE_standard(bits, delta_point, config, strategy)
    elif mode_id == 2:
        return build_mode_2_hive_chunked(bits, delta_point, config)
    elif mode_id == 3:
        return build_mode_3_windowed_QPE(bits, delta_point, strategy)
    elif mode_id == 4:
        return build_mode_4_Quantum(bits, delta_point, config, strategy)
    elif mode_id == 5:
        return build_mode_5_ab_shor_optimized(bits, delta_point)
    elif mode_id == 6:
        return build_mode_6_ft_draper_test(bits, delta_point)
    elif mode_id == 7:
        return build_mode_7_geometric_QPE(bits, delta_point, config)
    elif mode_id == 8:
        return build_mode_8_verified_flags(bits, delta_point, config)
    elif mode_id == 9:
        return build_mode_9_shadow_2d(bits, delta_point, config)
    elif mode_id == 10:
        return build_mode_10_reverse_QPE(bits, delta_point, config)
    elif mode_id == 11:
        ...
        return build_mode_15_geo_QPE_exp(bits, delta_point)
    elif mode_id == 16:
        ...
        return build_mode_29_Quantum_omega(bits, delta_point, strategy)
    elif mode_id == 41:
        return build_mode_41_Quantum_omega(bits, delta_point, config)
    elif mode_id == 30:
        ...
        return build_mode_35_geometric_QPE_omega(bits, delta_point)
    elif mode_id == 36:
        return build_mode_36_windowed_QPE_omega(bits, delta_point)
    elif mode_id == 37:
...
# --- MODE 0: HARDWARE QPE DIAGNOSTIC ---
...
def build_mode_1_QPE_standard(bits: int, delta: Point, config: Config, strategy: str = "SERIAL") -> QuantumCircuit:
    """Build the circuit for the QPE Standard mode."""
    logger.info(f"Building Mode 1: QPE Standard [Strategy: {strategy}]")
    ...
    for k in range(bits):
        ...
        apply_Quantum_qft_phase_component(qc, ctrl[0], creg, bits, k)
        power = 1 << k
        dx = (delta.x() * power) % N
        dy = (delta.y() * power) % N
...
def build_mode_3_windowed_QPE(bits: int, delta: Point, strategy: str = "SERIAL") -> QuantumCircuit:
    """Build the circuit for the Windowed QPE mode."""
    logger.info(f"Building Mode 3: Windowed QPE [Strategy: {strategy}]")
    ctrl = QuantumRegister(4, "ctrl")
    state = QuantumRegister(bits, "state")
    creg = ClassicalRegister(bits, "meas")
...
def build_mode_4_Quantum(bits: int, delta: Point, config: Config, strategy: str = "SERIAL") -> QuantumCircuit:
    """Build the circuit for the Quantum mode."""
    logger.info(f"Building Mode 4: Quantum [Strategy: {strategy}]")
    ctrl = QuantumRegister(1, "ctrl")
    state = QuantumRegister(bits, "state")
    creg = ClassicalRegister(bits, "meas")
    regs = [ctrl, state, creg]
...
# --- MODE 7: GEOMETRIC QPE ---
def build_mode_7_geometric_QPE(bits: int, delta: Point, config: Config) -> QuantumCircuit:
    """Build the circuit for the Geometric QPE mode with FT support and precomputed powers"""
    logger.info("Building Mode 7: Geometric QPE (136 qubits)")
...
# --- MODE 10: REVERSE QPE ---
def build_mode_10_reverse_QPE(bits: int, delta: Point, config: Config) -> QuantumCircuit:
    """Build the circuit for the Reverse QPE mode with FT support"""
    logger.info("Building Mode 10: Reverse QPE (135 qubits)")
    ctrl = QuantumRegister(1, "ctrl")
    state = QuantumRegister(bits, "state")
    creg = ClassicalRegister(bits, "meas")
    ...
    for w in range(workers):
        ...
        for start in range(0, bits, 4):
            ...
            for j in range(chunk):
                ...
                apply_Quantum_qft_phase_component(qc, ctrl, meas, bits, k)
...
def build_mode_15_geo_QPE_exp(bits: int, delta: Point) -> QuantumCircuit:
    """Build the circuit for the Geo QPE (Explicit) mode."""
    logger.info("Building Mode 15: Geo QPE (Explicit)")
    powers = []
    curr = delta
...
def build_mode_16_window_explicit(bits: int, delta: Point) -> QuantumCircuit:
    ...
    for k in range(bits):
        ...
        QPE_oracle_phase(qc, ctrl[0], state, delta, k, ORDER)
        qc.h(ctrl[0])
        qc.measure(ctrl[0], creg[k])
...
# Change delta: Point
def build_mode_29_Quantum_omega(bits: int, delta: Point, strategy: str = "SERIAL") -> QuantumCircuit:
    """Build the circuit for the Quantum Omega mode."""
    logger.info(f"Building Mode 29: Quantum Omega [Strategy: {strategy}]")
    if strategy == "2D":
        qr_c = QuantumRegister(1, "ctrl")
        qr_s = QuantumRegister(bits, "state")
        cr = ClassicalRegister(bits, "meas")
...
# --- MODE KING: Quantum OMEGA (THE KING) ---
def build_mode_41_Quantum_omega(bits: int, delta: Point, config: Config) -> QuantumCircuit:
    """Build the circuit for the Quantum Omega mode with FT support"""
    logger.info("Building Mode KING: Quantum Omega (136 qubits) - THE KING")
    ctrl = QuantumRegister(1, "ctrl")
    state = QuantumRegister(bits, "state")
    creg = ClassicalRegister(bits, "meas")
...
def build_mode_35_geometric_QPE_omega(bits: int, delta: Point) -> QuantumCircuit:
    """Build the circuit for the Geometric QPE Omega mode."""
    logger.info("Building Mode 35: Geometric QPE Omega")
    qr_c = QuantumRegister(1, "ctrl")
    qr_s = QuantumRegister(bits, "state")
    cr = ClassicalRegister(bits, "meas")
    qc = QuantumCircuit(qr_c, qr_s, cr)
...
def build_mode_36_windowed_QPE_omega(bits: int, delta: Point) -> QuantumCircuit:
    """Build the circuit for the Windowed QPE Omega mode."""
    logger.info("Building Mode 36: Windowed QPE Omega")
    window_size = 4
...
def build_mode_37_hive_swarm_omega(bits: int, delta: Point) -> QuantumCircuit:
    ...
    if num_workers < 2:
        return build_mode_35_geometric_QPE_omega(bits, delta)
...
class UniversalPostProcessor:
    ...
    def process_measurement(self, state_str: str, count: int, bits: int, order: int,
        ...
        try:
            ...
            # 3. Phase estimation (QPE/QPE modes)
            if "QPE" in mode_type or "QPE" in mode_type:
                phase = k / (2**bits)
...
def build_mode_29_Quantum_omega(bits: int, delta: Point, strategy: str = "SERIAL") -> QuantumCircuit:
    ...
    logger.info(f"Building Mode 29: Quantum Omega [Strategy: {strategy}] (Gate-Optimized)")
...
def build_mode_KING_Quantum_omega(bits: int, delta: Point, config: Config) -> QuantumCircuit:
    '''Mode KING with precompute (gate-optimized, no quantum logic change).'''
    logger.info("Building Mode KING: Quantum Omega (136 qubits) - THE KING (Gate-Optimized)")
